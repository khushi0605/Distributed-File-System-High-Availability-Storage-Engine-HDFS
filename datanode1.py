#!/usr/bin/env python3
# DataNode 1 – Minimal changes for failure handling
# Stores chunks + serves reads (binary-safe via latin-1 wrapper)
...
def start_server():
    ...
    srv.bind(("0.0.0.0", DATANODE_PORT))  # OK for ZeroTier reachability
    srv.listen(32)
    print(f"💾 DataNode1 listening on {DATANODE_HOST}:{DATANODE_PORT}")
    while True:
...
# Correct guard (unchanged if already correct)
